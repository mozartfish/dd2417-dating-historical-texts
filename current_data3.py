...
def create_paragraph_data(clean_split_path, raw_data_path):
    ...
    for decade in years:
        ...
        if os.path.exists(decade_path):
            ...
            for index, text_filename in enumerate(text_files):
                ...
                print(f"length of a paragraph: {len(book_paragraphs[0])}")
    ...
    #         [f for f in os.listdir(decade_path) if f.endswith(".txt")]
    #     )
    #     text_file = text_files[4]
    #     decade_titles = book_titles[int(decade)]
    #     text_file_number = int(re.findall(r"\d+", text_file)[0])
    #     book_title = decade_titles[text_file_number - 1]
    #     book_path = os.path.join(decade_path, text_file)
    #     with open(book_path, 'r', encoding='utf-8') as f:
    #         clean_book_data = f.read()
    #         print(f"succesfully read book!")
    #     book_paragraphs = create_paragraphs(clean_book_data)
    #     print(f"paragraphs created: {len(book_paragraphs)}")
    #     # print(book_paragraphs)
    #     print(f"decade path: {decade_path}")
    #     print(f'book path: {book_path}')
    #     print(f"book filename: {text_file}")
    #     print(f"book_title: {book_title}")


_ = create_paragraph_data(clean_train_split_path, raw_dataset_path)
...
# %%
def write_paragraphs_to_csv(paragraphs_data, filepath):
    ...
    print(f"succesfully wrote {len(paragraphs_data)} -> {filepath}")


# %%
def create_paragraph_dataset(clean_split_path, output_csv):
    paragraphs_data = []
    decade_dirs = [
        d
        for d in os.listdir(clean_split_path)
        if os.path.isdir(os.path.join(clean_split_path, d))
    ]
    decade_dirs.sort()
    ...
    for decade_dir in decade_dirs:
        ...
        print(f"current decade: {decade}")
        decade_path = os.path.join(clean_split_path, decade_dir)

        decade_paragraphs = 0
        text_files = [f for f in os.listdir(decade_path) if f.endswith(".txt")]

        for text_file in text_files:
            file_path = os.path.join(decade_path, text_file)
            book_title = text_file.replace(".txt", "")
